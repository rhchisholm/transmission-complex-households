...
%     end
    ...
    for i = 1 : parameters.Ntimesteps - 1 % for each time step
        ...
        [AgentCharacteristics, parameters, SummaryStatistics] = ...
            ...
            KeepIndexAllAgents, SummaryStatistics, CurrentYear,i);
        ...
        % For transmission scenario with event-based migration switched on
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Now that simulation has finished, we collate contact data, and 
    % calculate and store contact distribution statistics
    
    % First, collate contact information in these arrays
    hhcomtotal = zeros(size(SummaryStatistics.Contacts,1),2);
    hhcomUtotal = zeros(size(SummaryStatistics.Contacts,1),2);
    
    % for each person:
    for j = 1:size(SummaryStatistics.Contacts,1) 
        con=[]; %Collects IDs of all contacts that occured in dwellings
        conc=[]; %Collects IDs of all contacts that occured outside dwellings
        for i = 1: size(SummaryStatistics.Contacts,2) % for each timestep
            con=[con;SummaryStatistics.Contacts{j,i,1}];
            conc=[conc;SummaryStatistics.Contacts{j,i,2}];
        end
        hhcomtotal(j,1) = length(con); %total contacts
        hhcomtotal(j,2) = length(conc);
        hhcomUtotal(j,1) = length(unique(con)); % unique contacts
        hhcomUtotal(j,2) = length(unique(conc));
    end
    
    % remove zero entries due to those who weren't infected:
    idx = find(AgentCharacteristics.Immune);   
    hhcomtotal = hhcomtotal(idx,:);
    hhcomUtotal = hhcomUtotal(idx,:);
    
    % Now, calculate contact distribution data (quantiles and mean)
    SummaryStatistics.HHContactStats = ...
        [quantile(hhcomtotal(:,1),[0.025 0.25 0.5 0.75 0.975]) mean(hhcomtotal(:,1),1);
        quantile(hhcomUtotal(:,1),[0.025 0.25 0.5 0.75 0.975]) mean(hhcomUtotal(:,1),1)]; 
                                    
    SummaryStatistics.ComContactStats = ...
        [quantile(hhcomtotal(:,2),[0.025 0.25 0.5 0.75 0.975]) mean(hhcomtotal(:,2),1);
        quantile(hhcomUtotal(:,2),[0.025 0.25 0.5 0.75 0.975]) mean(hhcomUtotal(:,2),1)];                                

    SummaryStatistics.Contacts = []; % Dont save all contact data, just distribution data
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [AgentCharacteristics, parameters, SummaryStatistics] = ...
        transmission(parameters, AgentCharacteristics, CurrentExposed,...
        CurrentInfectious, CurrentImmune,CurrentHousehold, ...
        KeepIndexAllAgents, SummaryStatistics,CurrentYear,timestep)
...
% Update population change due to migration (events)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [AgentCharacteristics, parameters] = leaveafterevent(AgentCharacteristics, parameters)
    ...
    % residency: 0, they are permanent resident of community, 2 they are
    % temporary visitor for a sporting match, 3 for a funeral
